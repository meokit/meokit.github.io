...
Podish 的目标很简单：在**禁止 JIT 的 iOS** 上，尽可能高效地运行 x86 Linux 程序。它不是完整系统模拟器（如 UTM），而是一个用户态容器——类似 iSH，但代码完全从头重写，并在多个维度上性能翻倍。
...
我的问题是：**解释器最快能有多快？** 能不能在不做 JIT 的前提下，靠极致的解释器设计和硬件感知优化，逼近 JIT 的性能？
...
解释器的核心目标是：**在不使用 JIT 的前提下，把 x86 模拟的开销压到最低。** 下面是四个最关键的设计决策。

### 1. 预解码 + Tail-call Dispatch

传统解释器通常有一个中央分发循环（`while (1) { decode; dispatch; execute; }`），每次执行完一条指令都要回到这个循环。我的做法是把分发尽量前移：
...
1. **预解码**：先把变长 x86 指令解码成固定长度的中间表示（`DecodedOp`，32 字节），按基本块顺序存放。
...
- 高位（40-47 位）：PF lazy state（存的是 source byte，不是 PF bit 本身）

为什么只 lazy PF？因为 CF/ZF/SF/OF 几乎每个 ALU 指令都要写，而且 `jcc`/`cmov`/`setcc` 几乎每条都要读，做 lazy 的存取开销不划算。但 PF 的求值需要查表，比普通 flags 贵，而且用到 PF 的条件跳转只有 `JP`/`JNP`（cond 10/11），比例很低，值得 lazy。

此外，在基本块内做静态 def-use 分析：如果某条指令写 flags，但后面没人读，就直接换成 no-flags Handler 变体（如 `AluAdd<T, false>`），编译器会把整个 flags 更新路径完全剪掉。
...
2. 把一串 `DecodedOp` 紧凑地放成可 tail-call 的执行流
...
**Block Linking**：如果一个基本块足够短、指令不跨页，而且控制流关系简单，就把后继块直接接到当前块尾部，减少块间跳转时的间接访存。

**Superopcode**：不是简单统计高频 bigram，而是围绕高频 anchor instruction 选种子，再看它和前后指令的 def-use 关系，只融合存在 `RAW` 依赖的组合。这个策略最终生成了约 256 个 superopcode，是性能第一次稳定跨过 `3000+ CoreMark` 的关键步骤之一。
...
我的思路不是"监听整个地址空间的写操作"，而是**复用 MMU 的权限系统**，把检测成本压进权限位：
...
**执行期竞争**：仅靠"写的时候失效 block cache"还不够。如果当前 EIP 正好落在被写的页上，解释器 tail-call 跳转到的下一条指令可能已经被改写。为此实现了 `ShouldInterceptExecWriteForSmc`：一旦检测到当前 EIP 所在页被修改，会立即 yield 并切换到**单指令安全模式**（`max_insts = 1`），保证"写操作"和"跳转到新代码"之间有一个明确的指令边界，避免竞争。
...
Copy-and-patch 的核心思路：先把 Opcode handler 预编译成 binary stencil，运行时只做"拷贝模板 + patch 常量/地址/跳转槽位"。理论上可以避开完整后端、寄存器分配和传统机器码生成器的复杂度（相关论文：[Copy-and-Patch Compilation](https://arxiv.org/abs/2011.13127)）。

由于我已经用 `preserve_none` + `[[musttail]]` 把解释器热路径压得很低，我尝试把 handler 变成 stencil 再 patch 参数，连接成机器码。预期 200%+ 的性能提升，**实际上比解释器还略慢**。
...
我实现了 `clone`/`fork`/`vfork` 和基本的 `pthread` 语义，但并发模型**不是操作系统意义上的多线程并行**：
